 ...
 * Many functions in this module accept a pointer to a cpu state (STATE_PTR)
 * structure. We cannot just forward declare that because state_t was commonly
 * defined by clients as an anonymous struct typedef.
 */
...
/* All these functions access CP0 registers. Access to CP0 registers is always
 * possible in kernel mode, or in user mode with CPU 0 bit _set_ in STATUS
 * register 
 */
 ...
 * mode: otherwise it causes a program trap. It may be used to start a new *
 * process                                                                 *
 ***************************************************************************/
...
extern unsigned int 
FORK(unsigned int entryhi, unsigned int status, unsigned int pc, STATE_PTR statep);
...
#endif /* !defined(_UMPS_LIBUMPS_H_) */
