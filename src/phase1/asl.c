...
semd_t* Semd_ric(semd_t **tmp, int* key){
	if(*tmp == NULL){
                return NULL;
        }
        if((*tmp)->s_key == key){
...
/*[1]
Descrizione:gestione della lista dei semd attivi*/
...
void insert(semd_t **testa,semd_t *elemento){
        if(*testa == NULL){
                *testa = elemento;
...
Descrizione:Viene inserito il PCB puntato da p nella
	...
	key. Se il semaforo corrispondente non e’ presente nella
	ASL, alloca un nuovo SEMD dalla lista di quelli liberi
	(semdFree) e lo inserisce nella ASL, settando I campi in
	maniera opportuna. Se non e’ possibile allocare un nuovo
	SEMD perche’ la lista di quelli liberi e’ vuota, restituisce
	TRUE. In tutti gli altri casi, restituisce FALSE.
*/
...
/*[4]
Descrizione:Ritorna il primo PCB dalla coda dei processi bloccati
	(s_ProcQ) associata al SEMD della ASL con chiave key. Se tale
	descrittore non esiste nella ASL, restituisce NULL. Altrimenti, restituisce
	l’elemento rimosso. Se la coda dei processi bloccati per il semaforo
	diventa vuota, rimuove il descrittore corrispondente dalla ASL e lo
